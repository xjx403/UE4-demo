...
FMIJointConstraint::FMIJointConstraint(FMIRigidBody* InA, FMIRigidBody* InB)
{
	A = InA;
	B = InB;
...
void FMIJointConstraint::Solve(bool bMoveSubRoots)
{
...
void FMIJointConstraint::RemoveStretch()
{
...
FVector FMIJointConstraint::GetPositionCorrection(FVector& OutBodyToA, FVector& OutBodyToB)
{
...
void FMIJointConstraint::ApplyRotationCorrection(FQuat PureRotA, FQuat PureRotB)
{
...
void FMIJointConstraint::UpdateJointLimits()
{
	FMIBoneSettings& J = B->J;

	// no limits at all
	if (J.X == EMILimitType::Free &&
		J.Y == EMILimitType::Free &&
		J.Z == EMILimitType::Free)
	{
		return;
	}
	...
	bool bLockX = (J.X == EMILimitType::Locked) || (J.X == EMILimitType::Limited && ((J.MaxX - J.MinX) < 2.0f));
	bool bLockY = (J.Y == EMILimitType::Locked) || (J.Y == EMILimitType::Limited && ((J.MaxY - J.MinY) < 2.0f));
	bool bLockZ = (J.Z == EMILimitType::Locked) || (J.Z == EMILimitType::Limited && ((J.MaxZ - J.MinZ) < 2.0f));
	...
	bool bLimitX = J.X == EMILimitType::Limited && !bLockX;
	bool bLimitY = J.Y == EMILimitType::Limited && !bLockY;
	bool bLimitZ = J.Z == EMILimitType::Limited && !bLockZ;
...
void FMIJointConstraint::RotateWithinLimits(
	float MinAngle,
	float MaxAngle,
	float CurrentAngle,
	FVector RotAxis,
...
void FMIJointConstraint::UpdateLocalRotateAxes(bool bX, bool bY, bool bZ)
{
...
void FMIJointConstraint::DecomposeRotationAngles()
{
	FQuat ARot = A->Rotation * A->RotationOrig.Inverse();
	FQuat BRot = B->Rotation * B->RotationOrig.Inverse();
...
float FMIJointConstraint::SignedAngleBetweenNormals(
	const FVector& From, 
	const FVector& To, 
...
FMIPinConstraint::FMIPinConstraint(FMIRigidBody* InBody, const FVector& InPinPoint)
{
	A = InBody;
...
void FMIPinConstraint::Solve(bool bMoveSubRoots)
{
...
FVector FMIPinConstraint::GetPositionCorrection(FVector& OutBodyToPinPoint) const
{
