...
#include "MIRigUnit_PBIK.h"
#include "Units/RigUnitContext.h"

//#pragma optimize("", off)

FMIRigUnit_PBIK_Execute()
{
	...
	if (Context.State == EControlRigState::Init)
	{
		...
		int NumEffectors = 0;
		for (const FMIPBIKEffector& Effector : Effectors)
		{
	...
	for (const FMIPBIKBoneSetting& BoneSetting : BoneSettings)
	{
		...
		if (MIPBIK::FMIBoneSettings* InternalSettings = Solver.GetBoneSettings(BoneIndex))
		{
	...
	for (int E = 0; E < Effectors.Num(); ++E)
	{
		...
		const FMIPBIKEffector& Effector = Effectors[E];
