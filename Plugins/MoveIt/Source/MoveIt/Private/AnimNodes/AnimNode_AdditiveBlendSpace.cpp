...
void FAnimNode_AdditiveBlendSpace::Evaluate_AnyThread(FPoseContext& Output)
{
	...
	if (FAnimWeight::IsRelevant(ActualAlpha))
	{
		...
		FAnimNode_BlendSpacePlayer::Evaluate_AnyThread(AdditiveEvalContext);
...
void FAnimNode_AdditiveBlendSpace::GatherDebugData(FNodeDebugData& DebugData)
{
	...
	FAnimNode_BlendSpacePlayer::GatherDebugData(DebugData);
}
...
void FAnimNode_AdditiveBlendSpace::UpdateInternal(const FAnimationUpdateContext& Context)
{
	if ((BlendSpace != NULL) && (Context.AnimInstanceProxy->IsSkeletonCompatible(BlendSpace->GetSkeleton())))
	{
		...
		FAnimNode_BlendSpacePlayer::UpdateInternal(Context);
	}
}
