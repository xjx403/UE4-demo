...
void UAnimGraphNode_AdditiveBlendSpace::ValidateAnimNodeDuringCompilation(class USkeleton* ForSkeleton, class FCompilerResultsLog& MessageLog)
{
	...
	// Blendspace
	UBlendSpaceBase* BlendSpaceToCheck = GetNode()->BlendSpace;
...
void UAnimGraphNode_AdditiveBlendSpace::BakeDataDuringCompilation(class FCompilerResultsLog& MessageLog)
{
	...
	GetNode()->GroupName = SyncGroup.GroupName;
	GetNode()->GroupRole = SyncGroup.GroupRole;
...
{
	UBlendSpaceBase* BlendSpace = GetNode()->BlendSpace;
	UEdGraphPin* BlendSpacePin = FindPin(GET_MEMBER_NAME_STRING_CHECKED(FAnimNode_BlendSpacePlayer, BlendSpace));
...
{
	if (GetNode()->BlendSpace)
	{
		HandleAnimReferenceCollection(GetNode()->BlendSpace, AnimationAssets);
	}
}
...
{
	HandleAnimReferenceReplacement(GetNode()->BlendSpace, AnimAssetReplacementMap);
}
...
void UAnimGraphNode_AdditiveBlendSpace::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	DetailBuilder.HideProperty(NodeHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FAnimNode_AdditiveBlendSpace, Z)));
}
